...
public class WebcamAndMicrophoneCapture
{
	...
	final private static int AUDIO_DEVICE_INDEX = 5;
	...
	public static void main(String[] args) throws Exception, org.bytedeco.javacv.FrameGrabber.Exception
	{
        ...
        // org.bytedeco.javacv.FFmpegFrameRecorder.FFmpegFrameRecorder(String filename, int imageWidth, int imageHeight, int audioChannels)
        // For each param, we're passing in...
        // filename = either a path to a local file we wish to create, or an RTMP url to an FMS / Wowza server
        ...
        FFmpegFrameRecorder recorder = new FFmpegFrameRecorder("rtmp://my-streaming-server/app_name_here/instance_name/stream_name", grabbedImage.width(), grabbedImage.height(), 2);
        ...
        // Key frame interval, in our case every 2 seconds -> 30 (fps) * 2 = 60 (gop length)
        ...
        new Thread(new Runnable() 
        {
			...
			public void run()
			{
				...
				try
				{
					...
					// Using a ScheduledThreadPoolExecutor vs a while loop with a Thread.sleep will allow
					// us to get around some OS specific timing issues, and keep to a more precise
					// clock as the fixed rate accounts for garbage collection time, etc
					// a similar approach could be used for the webcam capture as well, if you wish
					...
					exec.scheduleAtFixedRate(new Runnable() 
					{
						...
						public void run()
						{
							// Don't add audio until we have our start time from the cam capture loop
							// otherwise, the recorder timestamps will be out of whack
							if (startTime == 0)
								return;
        ...
        {
        	// Convert IplImage to org.bytedeco.javacv.Frame
            Frame convertedImage = converter.convert(grabbedImage);
            
            if (cFrame.isVisible())
            {
            	// Show our frame in the preview
            	cFrame.showImage(convertedImage);
            }

			// Let's define our start time...
            // This needs to be initialized as close to when we'll use it as possible,
            ...
            recorder.record(convertedImage);
        }
