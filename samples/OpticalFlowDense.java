 ...
 * DenseOpticalFlow between two images (Frame-1 & Frame-2) and put the velocity
 * of every pixel to another image (OF) in their coordinate.
 *
 * @author Dawit Gebreyohannes
 */
public class OpticalFlowDense {
	public static void main(final String[] args) {
		final Mat pFrame = imread("samples/image0.png", CV_LOAD_IMAGE_GRAYSCALE),
				cFrame = imread("samples/image1.png", CV_LOAD_IMAGE_GRAYSCALE), 
				pGray = new Mat(), cGray = new Mat(),
		...
		final FloatBuffer in = Optical_Flow.<FloatBuffer>createBuffer(), 
				out = OF.<FloatBuffer>createBuffer();

		final int height = pGray.rows(), 
				width = pGray.cols();
